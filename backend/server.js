...
app.delete('/api/todos/:id', async (req, res) => {
   try {
      const { id } = req.params;
      let idTodo = parseInt(id);

      // Thực thi câu lệnh SQL để xóa todo và trả về dữ liệu vừa xóa
      const result = await pool.query(
         'DELETE FROM todos WHERE id = $1 RETURNING *',
         [idTodo]
      );

      // Nếu rowCount = 0 nghĩa là không có todo nào mang id này trong database
      if (result.rowCount === 0) {
         return res.status(404).json({ error: 'Todo not found' });
      }

      // Xóa thành công, trả về dữ liệu của todo đã bị xóa
      res.status(200).json({ 
...
app.put('/api/todos/:id', async (req, res) => {
   try {
      ...
      // Thực hiện câu lệnh UPDATE trong PostgreSQL
      const result = await pool.query(
         'UPDATE todos SET title = $1, completed = $2 WHERE id = $3 RETURNING *',
         [title, completed, id]
      );

      // KIỂM TRA: Nếu không tìm thấy todo nào có ID hợp lệ để cập nhật
      if (result.rows.length === 0) {
         return res.status(404).json({ error: 'Todo not found' });
      }

      // Trả về dữ liệu sau khi đã cập nhật thành công (Status 200 OK)
